<template>
    <tr
        :key="tx.token"
        v-if="to5(toPercent(tx.share, toNumber(asset.positions.lpShares))) != 0"
    >
        <!-- {{ stakeinfo }} <br> {{ poolinfo }} <br> {{ reward }} -->

        <th scope="row">
            {{ asset.symbol }}
            <footer id="apr" class="blockquote-footer">
                Apr: {{ to3(asset.statistic.apr * 100) }}%
            </footer>
        </th>
        <td>
            <!--LP-->{{ to3(tx.share) }}/<!--Staked LP-->({{
                to3(parseInt(stakeinfo[0].bond_amount) / 1000000)
            }})
        </td>
        <td>
            <!--Withdrawal-->
            ({{
                to4(
                    (toPercent(tx.share, toNumber(asset.positions.lpShares)) *
                        toNumber(asset.positions.pool)) /
                        100
                )
            }}
            {{ asset.symbol }} <br />
            ...
            {{
                to4(
                    (toPercent(tx.share, toNumber(asset.positions.lpShares)) *
                        toNumber(asset.positions.uusdPool)) /
                        100
                )
            }}
            ust )
        ...
        <td>
            <!--Pool Share-->
            {{ to4(toPercent(tx.share, toNumber(asset.positions.lpShares))) }}
        </td>
        <td>
            <!--LP Value or Witdrawal Value-->{{
                to3(
                    ((toPercent(tx.share, toNumber(asset.positions.lpShares)) *
                        toNumber(asset.positions.uusdPool)) /
                        100) *
        ...
        <td>
            <!--Cost Value-->
            {{ to4(tx.ust * 2) }} ust <br /><!-- Cost Price-->
            ({{ to4(tx.ust / tx.masset) }} ust)
        </td>
        <td>
            <!--Swap Price-->
            {{
                to3(
                    (toPercent(tx.share, toNumber(asset.positions.lpShares)) *
                        ...
                        ((toPercent(
                            tx.share,
                            toNumber(asset.positions.lpShares)
                        ) *
                            toNumber(asset.positions.pool)) /
                            100)
                )
        ...
        <td>
            <!--Unclaimed-->
            {{ to4(reward) }} mir <br />
            ({{ to4(reward * Number(mir.prices.price)) }} ust )
        </td>
        <!--
...
export default {
    ...
    methods: {
        ...
        rewardInfo: async function(address, token) {
            let txt = "";
            const rewardData = await mirror.staking.getRewardInfo(
                address,
                token
            );
            ...
            for (const [key, tokenData] of Object.entries(rewardInfos)) {
                console.log(key);
                const poolData = await mirror.staking.getPoolInfo(
                    tokenData.asset_token
                );
                ...
                const rewardToClaim =
                    (Number(poolData.reward_index) - Number(tokenData.index)) *
                        Number(tokenData.bond_amount) +
                    Number(tokenData.pending_reward);
