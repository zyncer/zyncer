<template>
    <div id="portfolio">
        ...
        <h4 class="font-weight-bold alert alert-success" v-if="waddress">
            Total Unclaimed: {{ to4(totalreward) }} mir (Price:{{
                Number(mir.prices.price)
            }}
            ust)
            <footer class="blockquote-footer">
                ({{ to4(totalreward * Number(mir.prices.price)) }} USD ~
                {{ to4(totalreward * Number(mir.prices.price) * 33) }} THB)
            </footer>
        </h4>
        <table
            v-if="waddress"
    ...
    computed: {
        ...
        ,{})*/
            ...
            this.txs
                .filter(
                    (tx) =>
                        tx.type === "PROVIDE_LIQUIDITY" ||
                        tx.type === "WITHDRAW_LIQUIDITY"
                )
                .sort((a, b) => {
                    if (
                        a.type === "PROVIDE_LIQUIDITY" &&
                        b.type === "WITHDRAW_LIQUIDITY"
                    )
                        return -1;
                    if (
                        a.type === "WITHDRAW_LIQUIDITY" &&
                        b.type === "PROVIDE_LIQUIDITY"
                    )
                        return 1;
                    return 0;
                })
                .forEach((item) => {
                    ...
                    if (sumsArray[item.token]) {
                        if (item.type === "PROVIDE_LIQUIDITY") {
                            sumsArray[item.token].share +=
                                parseInt(item.data.share) / 1000000;
                            sumsArray[item.token].masset +=
                                parseInt(
                                    item.data.assets.substr(
                                        0,
                                        item.data.assets.indexOf("terra")
                                    )
                                ) / 1000000;
                            sumsArray[item.token].ust +=
                                parseInt(
                                    item.data.assets.substr(
                                        item.data.assets.indexOf(",") + 1,
                                        item.data.assets.indexOf("uusd") -
                                            1 -
                                            item.data.assets.indexOf(",")
                                    )
                                ) / 1000000;
                        } else {
                            sumsArray[item.token].share -=
                                parseInt(item.data.withdrawnShare) / 1000000;
                            sumsArray[item.token].masset -=
                                parseInt(
                                    item.data.refundAssets.substr(
                                        item.data.refundAssets.indexOf(",") + 1,
                                        item.data.refundAssets.indexOf(
                                            "terra"
                                        ) -
                                            item.data.refundAssets.indexOf(
                                                ","
                            ...
                            sumsArray[item.token].ust -=
                                parseInt(
                                    item.data.refundAssets.substr(
                                        0,
                                        item.data.refundAssets.indexOf("uusd")
                                    )
                                ) / 1000000;
                        ...
                        sumsArray[item.token] = {
                            token: item.token,
                            share: parseInt(item.data.share) / 1000000,
                            masset:
                                parseInt(
                                    item.data.assets.substr(
                                        0,
        ...
        LPStaked: function() {
            let result = {
                all: this.assets.reduce(
                    (acc, item) =>
                        acc + parseInt(item.positions.lpStaked) / 1000000,
                    0
                ),
                my: this.txs
